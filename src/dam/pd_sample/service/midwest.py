...
class Midwest(object):
    ...
    def __init__(self):
        self.mid = pd.read_csv('../../../../static/midwest.csv')
        self.my_mid = None
...
MIDWEST_MENUS = ["종료",  # 0
                  "메타데이터 출력",  # 1
                  "메타데이터 한글화",  # 2
                  "전체 인구 대비 아시아 인구 백분율 변수 추",  # 3 18세이상만 사용함
                  "아시아 인구 백분율 전체 평균을 large/small 로 분류",  # 4
                  "large/small 빈도표와 빈도막대그래프 작성",  # 5
                    ]  # 9

midwest_menu = {

    "1": lambda t: t.print_meta(),
    "2": lambda t: t.rename(),
    "3": lambda t: t.drived_variable(),
    "4": lambda t: t.judge(),
    "5": lambda t: t.graph(),

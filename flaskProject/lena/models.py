...
class Gray(object):
    def __init__(self,path):
        ...
        self.path = path
        self.img = Image.open(BytesIO(requests.get(self.path, headers={'User-Agent': 'My User Agent 1.0'}).content))

    def get(self):
        img = np.array(self.img)
        img = img[:, :, 0] * 0.114 + img[:, :, 1] * 0.587 + img[:, :, 2] * 0.229
        return img
...
class CannyModel(object):
    def __init__(self,path):
        ...
        res = requests.get(self.path, headers=headers)
